---
title: "Docker - create a containner on your computer" 
date: 2019-10-20
...
Docker, written in the Go language, is a set of platform to develop, ship and 
...
[https://docs.docker.com/install/linux/docker-ce/ubuntu/](here).

First, we will uninstall older versions of Docker (`docker`, `docker.io` or
`docker-engine`). To do this, we enter the following to the console: 
```sh 
...
1. First, we update the `apt` pkacage index and install the relevant packages
over HTTPS: 
```sh
sudo apt-get update
...
images of interest. The `rocker/r-base` image offers the R console. Using the
`rocker/rstudio` image, the user can access the R language together with the 
RStudio IDE (in this case a web-based service accessible locally via a 
browser). Some options need to be supplied, for instance options to map the 
RStudio port (normally 8787) to a local system port (also normally 8787). 
